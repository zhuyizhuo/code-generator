...
<#if tableInfo.singlePrimaryKey>
    ...
    int ${methodDescription.DELETE_BY_PRIMARY_KEY.methodName}(${paramType} ${methodParam});
<#else>
    <#include "comment/commonCommentPara.ftl"/>
    <#include "comment/methodComment.ftl"/>
    int ${methodDescription.DELETE_BY_PRIMARY_KEY.methodName}(${stratificationInfo.pojoName} ${methodParam});
</#if>
