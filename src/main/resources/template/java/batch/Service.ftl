...
/**
 * description : ${tableInfo.dbTableName} table service layer interface	<br/>
 */
public interface ${tableInfo.tableName}Service extends Serializable {
...
<#list tableInfo.javaColmBeans as colm>
	 ...
	 * 作者: ${author} <br>
	 * 时间: ${date?string("yyyy-MM-dd HH:mm:ss")}<br>
	 * @param ${tableInfo.tableName?uncap_first}List 存放实体的list <br>
	 * @param ${tableInfo.tableName?uncap_first} 存放参数的实体  <br>
<#list tableInfo.javaColmBeans as colm>
	 <#if colm??>
	 * @param ${colm.javaName} ${colm.comment} <br>
	 ...
	 */
	void batchInsert${tableInfo.tableName}(List<${tableInfo.tableName}> ${tableInfo.tableName?uncap_first}List);
